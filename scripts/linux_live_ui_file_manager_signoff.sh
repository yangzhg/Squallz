...
REPORT="${SQUALLZ_LINUX_LIVE_UI_FILE_MANAGER_REPORT:-"$ROOT/benches/LINUX_LIVE_UI_FILE_MANAGER_SIGNOFF.md"}"
MANIFEST="${SQUALLZ_LINUX_LIVE_UI_EVIDENCE_MANIFEST:-}"
...
required_bool_fields = [
    ...
    "packaged_launch",
    "archive_opened",
    "gnome_files_menu_visible",
    "dolphin_menu_visible",
    "outputs_verified",
...
required_screenshot_fields = [
    ...
    "gnome_files_context_menu_screenshot",
    "dolphin_context_menu_screenshot",
...
forbidden_tokens = [
    ...
    "static service-file inspection",
    "static service file inspection",
    "compile-only",
    "compile only",
...
def file_status(raw: Any, base: Path, *, screenshot: bool = False) -> tuple[str, Optional[Path], list[str], list[str]]:
    ...
    if screenshot and path.suffix.lower() not in {".png", ".jpg", ".jpeg"}:
        return "bad-extension", path, [], [f"screenshot must be PNG/JPEG: {path}"]
    return "present", path, [], []
...
if not manifest_arg:
    blocked.append(
        "SQUALLZ_LINUX_LIVE_UI_EVIDENCE_MANIFEST is not set; run this script with a manifest produced by target GNOME/KDE file-manager click signoff."
    )
else:
...
if manifest is not None and manifest_path is not None:
    ...
    for field in required_bool_fields:
        ...
        if value is not True:
            if field in {"target_os_linux", "actions_invoked_packaged_binary", "no_dev_server"} and value is False:
    ...
    if package_path is not None and package_path.is_file() and package_hash:
        ...
        if actual_hash.lower() == package_hash.lower():
            ...
            failures.append(f"package hash mismatch expected={package_hash.lower()} actual={actual_hash}")

    evidence_rows.append(
        f"| package artifact | {package_status} | `{md_cell(package_path or manifest.get('package_artifact_path', ''))}` |"
    )

    for field in ["install_log", "app_launch_log", "open_file_log", "file_manager_actions_log"]:
...
report_path.write_text(
    "\n".join(
        [
            "# Linux Live UI and File-Manager Integration Signoff",
            "",
            f"Generated: {now}",
            "",
            f"Status: {status}",
            ...
            "## Scope",
            "",
            "This report is the formal release blocker artifact for Linux live UI",
            "and file-manager integration. It validates evidence from target GNOME",
            "and KDE desktop sessions using the packaged Squallz artifact.",
            "",
            "It does not accept dev-server live-clicks, headless cargo tests,",
            "macOS package smokes, static service-file inspection, or CLI-only",
            "archive operations as a substitute for visible file-manager clicks.",
            "",
            "## Manifest Format",
            "",
            "Set `SQUALLZ_LINUX_LIVE_UI_EVIDENCE_MANIFEST=/path/to/manifest.json`.",
            "Relative paths are resolved from the manifest directory.",
            "",
            "Required top-level fields: `package_artifact_path`, `package_sha256`,",
            "`package_kind`, `machine_id`, `distro_version`, `desktop_sessions`,",
            "`file_managers`, `webkitgtk_version`, `runtime_version`, `install_log`,",
            "`app_launch_log`, `open_file_log`, `file_manager_actions_log`, four",
            ...
            f"- Distro: `{md_cell(manifest.get('distro_version', '-') if manifest else '-')}`",
            f"- Desktop sessions: `{md_cell(manifest.get('desktop_sessions', '-') if manifest else '-')}`",
            f"- File managers: `{md_cell(manifest.get('file_managers', '-') if manifest else '-')}`",
            f"- WebKitGTK/runtime: `{md_cell(manifest.get('webkitgtk_version', '-') if manifest else '-')}` / `{md_cell(manifest.get('runtime_version', '-') if manifest else '-')}`",
            ...
            *action_rows,
            ...
            "- Headless cargo tests or compile-only reports.",
            "- macOS package smoke, DMG install smoke, or package contents inspection.",
            "- Nautilus/Dolphin service-file inspection without visible file-manager clicks.",
